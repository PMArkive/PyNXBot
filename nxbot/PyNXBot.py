...
class BDSPBot(NXBot):
    ...
    def readParty(self, slot=1):
        ...
        partyPointer = f"[[[[[[[[[[[main+{self.playerPrefsProvider:X}]+18]+C0]+28]+B8]]+{self.partyPkmn:X}]+10]+{0x20 + (0x8 * (slot - 1)):X}]+20]+18]+20"
    ...
    def readBox(self, box=1, slot=1):
        ...
        boxPointer = f"[[[[[[[[[main+{self.playerPrefsProvider:X}]+18]+C0]+28]+B8]]+A0]+{0x20 + (0x8 * (box - 1)):X}]+{0x20 + (0x8 * (slot - 1)):X}]+20"
